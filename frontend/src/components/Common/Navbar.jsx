...
const Navbar = () => {
    ...
    return (
        <>
            <nav className="container mx-auto flex items-center justify-between py-4 px-6">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    {user && user.role === "admin" && (
                        <Link to="/ctrl" className="block bg-black px-2 rounded text-sm text-white">
                            Admin
                        </Link>
                    )}
                    {/* User profile icon */}
            ...
            {/* Mobile Navigation */}
            <div className={`fixed top-0 left-0 w-3/4 sm:w-1/2 md:w-3/4 h-full bg-white shadow-lg transform transition-transform duration-300 z-50 ${navDrawerOpen ? "translate-x-0" : "-translate-x-full"}`}>
